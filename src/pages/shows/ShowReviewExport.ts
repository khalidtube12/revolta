import type { ShowReview, Match } from '../../types';
...
export async function ShowReviewExport({ review, match, showName }: {
  review: ShowReview & { authorName: string; authorPhoto: string | null; authorTwitter: string | null };
  match: Match;
  showName: string;
}): Promise<void> {
  ...
  const [matchImg, logoImg, avatarImg] = await Promise.all([
    match.imageUrl ? loadImg(match.imageUrl) : Promise.resolve(null),
  ...
  // match image
  if (matchImg) {
    const ratio = Math.max(W / matchImg.width, IMG_H / matchImg.height);
    const pw = matchImg.width * ratio, ph = matchImg.height * ratio;
    ctx.save();
    ctx.beginPath(); ctx.rect(0, 0, W, IMG_H); ctx.clip();
    ctx.globalAlpha = 0.75;
    ctx.drawImage(matchImg, (W - pw) / 2, (IMG_H - ph) / 2, pw, ph);
    ctx.globalAlpha = 1;
    ctx.restore();
  }
  ...
  // stars row (bottom-left of image)
  const starSz = 18;
  ...
  // match title — center-aligned with stars row
  const starsMid = starTop + starSz / 2;
  ctx.direction = 'rtl'; ctx.textAlign = 'start';
  ctx.font = '700 22px Cairo, sans-serif'; ctx.fillStyle = '#fff';
  ctx.textBaseline = 'middle';
  ctx.shadowColor = 'rgba(0,0,0,0.9)'; ctx.shadowBlur = 16;
  ctx.fillText(match.title, W - PAD, starsMid);
  ctx.shadowBlur = 0;

  // verdict (left — opposite show name)
  if (review.verdict) {
    ctx.direction = 'ltr'; ctx.textAlign = 'left';
  ...
  // show name — just above match title
  ctx.direction = 'rtl'; ctx.textAlign = 'start';
  ctx.font = '400 13px Cairo, sans-serif'; ctx.fillStyle = 'rgba(201,168,76,0.75)';
  ctx.textBaseline = 'bottom';
  ctx.shadowColor = 'rgba(0,0,0,0.9)'; ctx.shadowBlur = 10;
  ctx.fillText(showName, W - PAD, starTop - 8);
  ctx.shadowBlur = 0;

  // gold bar
  let y = IMG_H;
  const barGrad = ctx.createLinearGradient(0, 0, W, 0);
  barGrad.addColorStop(0, 'transparent'); barGrad.addColorStop(0.4, '#c9a84c'); barGrad.addColorStop(1, '#c9a84c');
  ctx.fillStyle = barGrad; ctx.fillRect(0, y, W, 2);
  y += 2 + PAD;
  ...
  link.download = `revolta-${match.title.replace(/\s+/g, '-')}.png`;
